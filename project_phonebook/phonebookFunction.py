...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"): #(title, message)
        # This closes app
        self.master.destroy()
        os._exit(0) #clears out the memory
...
def count_records(cur):
    ...
    count = cur.fetchone()[0] #this is extracting/fetching the data
    return cur,count


#Select item in ListBox (called in GUI under <<ListboxSelect>>)
def onSelect(self,event):
    ...
    with conn:
        ...
        for data in varBody:
            self.txt_fname.delete(0,END) #(0,END) deletes the whole thing
...
def addToList(self):
    ...
    print("var_fullname: {}".format(var_fullname)) #won't be seen by user
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                print("chkName: {}".format(chkName)) #just for checking - remove if going live
